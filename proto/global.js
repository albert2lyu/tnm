...
TNM.Global.monitorCaller = function(name, params, cbFn, failFn){
	...
	case "getZonesForTree":
		...
		*/
		setTimeout(function(){cbFn(Test.getZonesForTree(params));}, 100);
		break;
	case "getCameras":
		/**
			params: {siteId, zoneId, status// "all"(全部), 1(在线), 0(未在线)}
			cbFn([{
				total: 
				items: [{
					id, name, zoneId, status
				}]
			}]);
		*/
		setTimeout(function(){cbFn(Test.getCameras(params));}, 100);
		break;
	case "getPerf":
		/* params : {id: deviceId}
			cbFn({mem : [totalInGB, usedInGB], cpu : [100, percentage], disk : [totalInGB, usedInGB]})
		*/
		return setTimeout(function(){cbFn(params.id == 5 ? {
			// mem : [4, 3], 
			// cpu : [100, 70], 
			// disk : [3000, 2000]

			mem : [400, randomInt(400)], 
			rw : [1000, randomInt(1000)], 
