...
function SiteDevices(site){
	...
	function getAllZoneCameras(siteId){
		...
		for (var i = 0; i < DeviceZones.length; i++) {
			...
			zoneData.cameras = IX.loop(cameras, [], function(acc, c){
				if (c.zoneId == zoneId)
					acc.push({id : c.id, type:c.type, name:c.name, zoneId: c.zoneId});
				return acc;
			});
	...
	//列出所有监视信息里的设备,目前只有服务器和存储设备
	function getDevicesForMonitor(siteId){
		var DevicesTypes = [1, 2, 3, 10, 20, 21, 22, 31, 32, 33, 40, 41, 50, 51, 52, 53, 54, 55, 60, 61, 62, 63, 64, 65, 90];
		if(siteType == 1)
			DevicesTypes = [0, 1, 2, 3, 10, 51, 52, 53, 54, 55, 60, 61, 62, 63, 64, 90];
		var allDevices = [];
		var items=[{id:1,name:"服务器1"},{id:2,name:"服务器2"},{id:3,name:"服务器1"},{id:4,name:"服务器4"},{id: 5, name: "存储设备"}];
		for (var i = 0; i < DevicesTypes.length; i++) {
			allDevices.push({
				type: DevicesTypes[i],
				count: Devices4All.get(DevicesTypes[i]).length,
...
Test.getAllCameras = function(){
	var _site = Test.getCurrentSite();
	if (_site.type == 2 || _site.type == 3)
		return [getSiteZoneAndCameras(_site.id)];

	return IX.loop(Test.getAllSites(), [], function(acc, site){
...
Test.getZonesForTree = function(params){
	var sites = TCM.LineInfo.getSites().getAll();
	var zones = Test.getAllZones();
	var result = IX.map(sites, function(site, idx){
		site.zones = IX.loop(zones, [], function(acc, zone, _idx){
...
Test.getCameras = function(params){
	if (params.zoneId) {
		params.id = params.zoneId;
		params.types = [20,21,22,31,32,33];
		return Test.getCamerasByZone(params);
	}else {
		var items = IX.loop(getSiteZoneAndCameras(params.siteId)[1], [], function(acc, zones, idx){
			IX.iterate(zones.cameras, function(camera, idx){
				camera.status = idx % 3 === 1 ? 0 : 1;
				camera.ip = "192.168.1."+idx;
				acc.push(camera);
			});
			return acc;
		});
		return getPagedData(items, params.pageNo, params.pageSize);
	}
};
})();
