...
["cameraName", {name: "name", title: "摄像机名称", isLongStr: true}],
["camera4Zone", function(){return getCommonColumnModel("zoneId", "所属分区", function(item){
	return TNM.Monitor.getZoneName(item.zoneId);
}, true);}],
["cameraType", function(){return getCommonColumnModel("type", "摄像机类型", function(item){
	return TCM.Const.DeviceTypeNames[item.type];
}, true);}],
["cameraIp", {name: "ip", title: "摄像机IP", isLongStr: true}],
["cameraStatus", function(){return getCommonColumnModel("status", "摄像机在线状态", function(item){
	return item.status ? "在线" : "未在线";
}, true);}]
...
var filterToolsCfgHT = {
	...
	"status": {
		id: "status",
		items: [{id: "all", name: "全部"}, {id: 1, name: "在线"}, {id: 0, name: "未在线"}]
	}
};
...
function refreshProgress(pEl, fromAngel, toAngel){
	...
	if (deviceType == 10) {
		jQuery(".mem span.s").css("display", "none");
		jQuery(".mem .info").css("paddingLeft", "0px");
		jQuery(".rw .info span:last").css("display", "none");
	}
}
...
var itemHT = {
	...
	"storage" : [
		{type: "mem", value : 0, unit : "", title : "网口吞吐速率（Mbps）"}, 
		{type : "rw", value : 0, unit : "", title : "磁盘读写速率（Mbps）"},
		{type : "disk", value : 0, unit : "GB", title : "磁盘（共{total}GB）"}
	]
};
...
IX.ns("TNM.Lib");
TNM.Lib.DashboardPanel = function(_container, dataCaller, type){
	var container = $X(_container);
	deviceType = type;
	if (deviceType == 10) 
		DashboardItems = itemHT.storage;
	else
		DashboardItems = itemHT.service;
	container.innerHTML = t_panel.renderData("", {items : DashboardItems}).replaceAll("{total}", 0);
...
var t_zoneInfo = new IX.ITemplate({tpl: [
	'<div id="Tree" class="{clz}"><div id="treeName">{treeName}</div>{treeData}</div>',
	'<div id="zoneGrid"></div>',
'']});
...
var checker = null;
function getDevices(cbFn){
	deviceCaller("getDevicesForMonitor", {types:[0,1,2,3,10]}, function(data){
...
function resizeFn(){
	...
	if(width <= 1920 && width >= 1300){
		...
		jQuery(".rw").css({top: _calc(width, 30, 100), left: _calc(width, 245, 475)});
...
function zoneTreeClick(_params){
	var grid = null;
	var columns = "cameraName,camera4Zone,cameraType,cameraIp,cameraStatus".split(",");
	currentFilter = new TNM.Lib.BaseFilterTools({
		buttons: [],
		hasDate: false,
		toolList: [{
	...
	grid = new TNM.Lib.FilterGrid({
		...
		tools: currentFilter,
		usePagination : true,
		baseParams: {"status": "all"},
		dataLoader : function(params, _cbFn){
			monitorCaller("getCameras", IX.inherit(params, _params), function(data){
				_cbFn(data);
				$X("Tree").style.height = jQuery(".cameras-list").height() + "px";
			});
		}
	});
	return grid;
...
function getZones(cbFn){
	monitorCaller("getZonesForTree", {}, function(data){
		cbFn({nodes : IX.loop(data, [], function(acc, site){
			var obj = {};
			var keys = [];
			obj.nodes = IX.loop(site.zones, [], function(arr, zone){
				var zoneId = zone.id;
				keys.push(zoneId);
				zoneHT[zoneId] = zone.name;
				arr.push({key: [zoneId], name: zone.name, siteId: site.id, zoneId: zoneId, nodes: []});
				return arr;
			});
			obj.key = keys;
			obj.name = site.name;
...
function showCameraStatus(cbFn){
	getZones(function(_cfg){
		var cfg = IX.inherit(_cfg, {
			htmlFn : function(node) {return node.name;},
			click : zoneTreeClick
		});
		var tree = new TCM.Lib.Tree(cfg);
		$X('body').innerHTML = t_zoneInfo.renderData("", {
			treeName : "线路分区列表",
			treeData : tree.getHTML(),
			clz : "monitor"
		});
...
TNM.Monitor.getZoneName = function(zoneId){return zoneHT[zoneId];};
})();
(function () {
...
var kickChecker = new TNM.Util.KickChecker("", function(cbFn){
